        ...
        public class CommandHandler : IRequestHandler<Command, CommandResponse>
        {
            ...
            public Task<CommandResponse> Handle(Command message, CancellationToken cancellationToken)
            {
                ...
                using (var scope = _scopeFactory.Create())
                {
                    ...
                    isValid = context.Set<Idol>().Any(f =>f.NickName.Equals(message.NickName, StringComparison.OrdinalIgnoreCase));

                    if (!isValid)
                    {
                        var idol = Mapper.Map<Idol>(message);
                        context.Set<Idol>().Add(idol);
                        isValid = true;
                    }                    
